        ...
        public void gerer()
        {
            if (this.Virtuel == false) // On regarde si on se trouve en mode reel
            {
                ...
                else
                {
                    ...
                    foreach (TPiece piece in this.ListePieces)
                    {
                        ...
                        if ((piece.OuvertureExtOuverte() || (piece.OuvertureCommuneOuvert() != null && piece.OuvertureExtPieceVoisine())))
                        {
                        ...
                        else
                        {
                            if (piece.Consigne > piece.Temperature)
                            {
                                ...
                                {
                                    int pinChauffageSalon = 1; // 1 en sortie (pin 22)
                                    ULStat = DaqBoard.DBitOut(MccDaq.DigitalPortType.FirstPortA, pinChauffageSalon, MccDaq.DigitalLogicState.High);
                                }
                                else if (piece.Nom == "Chambre")
                                {
                                    int pinChauffageChambre = 3; // 3 en sortie (pin 24)
                                    ULStat = DaqBoard.DBitOut(MccDaq.DigitalPortType.FirstPortA, pinChauffageChambre, MccDaq.DigitalLogicState.High);
                                }
                                else if (piece.Nom == "Cuisine")
                                {
                                    int pinChauffageCuisine = 0; // 0 en sortie (pin 21)
            ...
            else
            {
                foreach (TPiece piece in this.ListePieces)
                {
                    ...
                    {
                        TParoi paroi = piece.OuvertureCommuneOuvert();
