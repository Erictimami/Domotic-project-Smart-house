...
namespace domotique
{
    ...
    {
        private Boolean statusBDD; // Status de connection BDD
        ...
        {
            var me = this;
            maMaison = new TMaison(this, false, 10, ref textBoxTExterieurActuelle, true, trackBar, statusStrip1, radioButtonReel, radioButtonVirtuel);

            TPiece monSalon = new TPiece("Salon", false, 15, 270, 1500, ref textBoxTSalonActuelle, ref textBoxTSalonConsigne, ref buttonChauffeSalon, con, labelHeure, labelDate, ref statusBDD, maMaison);
            TPiece maChambre = new TPiece("Chambre", false, 20, 150, 1000, ref textBoxTChambreActuelle, ref textBoxTChambreConsigne, ref buttonChauffeChambre, con, labelHeure, labelDate, ref statusBDD, maMaison);
            TPiece maCuisine = new TPiece("Cuisine", false, 20, 120, 500, ref textBoxTCuisineActuelle, ref textBoxTCuisineConsigne, ref buttonChauffeCuisine, con, labelHeure, labelDate, ref statusBDD, maMaison);
        ...
        private void clearLogsT(SqlConnection ACon)
        {
            if (statusBDD == true)
            {
                try
                {
                    String maRequete;
                    maRequete = "Delete from domotique.dbo.LogT wehe FK_mode = 1";
                    SqlCommand myCommand = new SqlCommand(maRequete, con);
        ...
        private void timerHeure_Tick(object sender, EventArgs e)
        {
            String dt2;
            int intervalle = 0;
            Boolean changeHeure = true;
            if (maMaison.Virtuel == true)
            {
                changeHeure = true;
        ...
        {
            Boolean existeDeja = false; // Permet de savoir si consigne existe deja
            DateTime date;
            String dt;
            ...
            {
                if ((maConsigne.Jour == cons.Jour) && (maConsigne.Heure == cons.Heure)) // Test si consigne existe deja
                {
                    existeDeja = true;
                    ...
                    {
                        if (statusBDD == true) // Si connexion ok on delete et ajoute la nouvelle
                        {
                            String maRequete;
            ...
            if (existeDeja == false) // Si elle n'existais pas deja
            {

                if (statusBDD == true) // Si connexion BDD OK on ajoute
                {
                    String maRequete, maRequete2;
                    SqlDataReader DataReader;
                    maRequete2 = "SELECT PK_Piece FROM domotique.dbo.Piece where nom = '" + maMaison.ListePieces[tabControlPieces.SelectedIndex].Nom + "'";
                    SqlCommand myCommand = new SqlCommand(maRequete2, con);
                    DataReader = myCommand.ExecuteReader();
                    DataReader.Read();
                    int idPiece = DataReader.GetInt32(0);
                    DataReader.Close();
                    maRequete = "INSERT INTO domotique.dbo.Consigne VALUES ('" + maConsigne.Jour + "' , '" + maConsigne.Heure + "', '" + maConsigne.Temperature + "', '" + idPiece + "')";
                    SqlCommand myCommand2 = new SqlCommand(maRequete, con);
                    myCommand2.ExecuteNonQuery();
                    maMaison.ListePieces[tabControlPieces.SelectedIndex].AjouterConsigne(maConsigne);
                }
                else
            ...
            maMaison.ListePieces[tabControlPieces.SelectedIndex].TrierListeConsignes(); // On trie la liste de consigne
            AfficherConsignes(); // On les affiches
        }
        ...
        /// <summary>
        /// Fonction qui s'éxécute lorsque l'on rentre dans la page Logs. On va donc afficher ceux-ci
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        /*private void tabPageLogs_Enter(object sender, EventArgs e)
        {
            ...
            SqlDataReader DataReader;
            int mode = 0;
            if (this.maMaison.Virtuel == true) // Test mode de la maison
            {
                mode = 1;
            }
            else
            ...
            maRequete = "SELECT PK_LogPiece, date, etat, Ouverture.nom, Piece.nom, pp.nom  from domotique.dbo.LogPiece INNER JOIN domotique.dbo.Ouverture ON FK_Ouverture = PK_Ouverture INNER JOIN domotique.dbo.Piece ON FK_Piece1 = PK_Piece INNER JOIN domotique.dbo.Piece as pp ON FK_Piece2 = pp.PK_Piece where FK_Mode = '" + mode + "'";
            SqlCommand myCommand = new SqlCommand(maRequete, con);
            DataReader = myCommand.ExecuteReader();
            DateTime date;
            String dt, heure, etat;
            int ouvert;
            dataGridViewLogPiece.Rows.Clear(); // On efface la tableau

            while (DataReader.Read()) // On charge le tableau
            {
        ...
        /// <summary>
        /// Permet l'import des logs vers Excell
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void buttonExcellOv_Click(object sender, EventArgs e)
        {
            // Eléments d'EXCEL
            ...
            oXL = new Excel.Application(); // OUVERTURE EXCEL
            ...
            oRng = oSheet.get_Range("A1", "F1"); // sélection première ligne
            oRng.Font.Bold = true; // et passage en GRAS
            for (int k = 0; k < dataGridViewLogPiece.ColumnCount; k++)
            {
                oSheet.Cells[1, k + 1] = dataGridViewLogPiece.Columns[k].HeaderCell.Value.ToString();
            }

            for (int i = 0; i < dataGridViewLogPiece.Rows.Count - 1; i++)
            {
                for (int j = 0; j < dataGridViewLogPiece.ColumnCount; j++)
                {
                    oSheet.Cells[i + 2, j + 1] = dataGridViewLogPiece[j, i].Value.ToString();
                }
            }
        ...
        private void afficherLogsOuvertures()
        {
            String maRequete;
            SqlDataReader DataReader;
            int mode = 0;
        ...
        private void afficherLogsT()
        {
            ...
            maRequete = "Select date, temperatureOld, temperature, nom from  domotique.dbo.LogT INNER join domotique.dbo.Piece ON PK_Piece = FK_Piece where FK_Mode = '"+mode+"'";
            SqlCommand myCommand = new SqlCommand(maRequete, con);
            DataReader = myCommand.ExecuteReader();
            DateTime date;
            String dt, heure, etat;
            dataGridViewLogsT.Rows.Clear(); // On efface la tableau
            int tempOld, temp = 0;
            while (DataReader.Read()) // On charge le tableau
            {
                date = DataReader.GetDateTime(0);
                tempOld = Convert.ToInt32(DataReader.GetValue(1));
                temp = Convert.ToInt32(DataReader.GetValue(2));

                heure = String.Format("{0:HH:mm:ss}", date);
                dt = String.Format("{0:d/MM/yyyy}", date);
                dataGridViewLogsT.Rows.Add(dt, heure, tempOld, temp, DataReader.GetValue(3));
            }
            DataReader.Close();
        }
        ...
        private void tabPageLogs_Enter(object sender, EventArgs e)
        {
            if (tabControlLogs.SelectedIndex == 0)
            {
                afficherLogsOuvertures();
            }
            else
            {
                afficherLogsT();
            ...
            Debug.Print(tabControlLogs.SelectedIndex.ToString());
        }

        private void buttonExcellT_Click(object sender, EventArgs e)
        {
            // Eléments d'EXCEL
            Excel.Application oXL; // Objet Excel application
            Excel.Workbook oWB; // Classeur
            ...
            oRng = oSheet.get_Range("A1", "E1"); // sélection première ligne
            oRng.Font.Bold = true; // et passage en GRAS
            for (int k = 0; k < dataGridViewLogsT.ColumnCount; k++)
            {
                oSheet.Cells[1, k + 1] = dataGridViewLogsT.Columns[k].HeaderCell.Value.ToString();
            }

            for (int i = 0; i < dataGridViewLogsT.Rows.Count - 1; i++)
            {
                for (int j = 0; j < dataGridViewLogsT.ColumnCount; j++)
                {
                    oSheet.Cells[i + 2, j + 1] = dataGridViewLogsT[j, i].Value.ToString();
                }
            }
