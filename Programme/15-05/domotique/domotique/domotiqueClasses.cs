...
namespace domotique
{
    ...
    public class TMaison : System.Object
    {
        ...
        public void gerer()
        {
            ...
            {
                if (this.ULStat.Value.ToString() != "NoErrors") // On test la connexion du DAQ
                {
                    if (msgError == false)
                    {
                ...
                else
                {
                    ...
                    foreach (TPiece piece in this.ListePieces)
                    {
                        ...
                        if ((piece.OuvertureExtOuverte() || (piece.OuvertureCommuneOuvert() != null && piece.OuvertureExtPieceVoisine() != null))) // Si ouverture ext ouverte ou commune + ext pièce voisine
                        {
                        ...
                        else // Sinon teste de consignes
                        {
                            if (piece.Consigne > piece.Temperature) // Si consigne plus grand -> allume chauffage
                            {
            ...
            else // Si mode virtuel
            {
                foreach (TPiece piece in this.ListePieces)
                {
                    if (piece.OuvertureExtOuverte()) // Si ouvertureExt ouverte
                    {
                        piece.Radiateur = false; // Coupe chauffage
                        piece.Temperature = this.temperatureExt; // Température pièce = température extérieur
                    }
                    else
                    {
                        TParoi paroiCommune = piece.OuvertureCommuneOuvert();
                        if ((paroiCommune != null)) // Si paroi commune ouverte
                        {
                            TPiece pieceVoisine = piece.OuvertureExtPieceVoisine();
                            if (piece.OuvertureExtPieceVoisine() != null) // Si pièce à une pièce voisine dont ouvertureEXt ouvert
                            {
                                if (pieceVoisine == paroiCommune.Piece1 || pieceVoisine == paroiCommune.Piece2) // On regarde si lien entre les 2 pièce
                                {
                                    piece.Radiateur = false; // Si oui on coupe
                                }
                                else if (pieceVoisine.OuvertureCommuneOuvert() != null) // Sinon si piècevoisine (qui a ouvertureExt ouverte) a une ouvertueCommune ouverte
                                {
                                    piece.Radiateur = false; // On coupe chauffage
                                }
                                else
                                {
                                    //piece.Radiateur = true; // Sinon on allume
                                    if (piece.Consigne > piece.Temperature) //Test consigne
                                    {
                                        piece.Radiateur = true;
                                    }
                                    else
                            ...
                            else // Si pas de pièce voisine ouvertureExt ouverte
                            {
                                if (piece.Consigne > piece.Temperature) //Test consigne
                                {
                                    piece.Radiateur = true;
                        ...
                        else // Sinon (pas d'ouvertures commune ni vers ext)
                        {
                            if (piece.Consigne > piece.Temperature) // test consignes
                            {
                                piece.Radiateur = true;
                            }
                            else
                        ...
                        {
                            if (paroi.Piece1 == piece) // Si on se trouve dans pièce 1
                            {
                                if (paroi.Piece2 != null) // Si pièce 2 != de l'extérieur
                                {
                                    apport = apport + ((paroi.Piece2.Temperature - piece.Temperature) * paroi.SurfaceReel) / paroi.CoefficientThermo;
                                }
                                else // Si c'est l'extérieur
                                {
                                    apport = apport + ((this.temperatureExt - piece.Temperature) * paroi.SurfaceReel) / paroi.CoefficientThermo;
                                }

                            }
                            else // Si on se trouve dans pièce 2
                            {
                                if (paroi.Piece1 != null) // Si piece 1 n'est pas extérieur
                                {
                                    apport = apport + ((paroi.Piece1.Temperature - piece.Temperature) * paroi.SurfaceReel) / paroi.CoefficientThermo;
                                }
                                else // Si pièce 1 est extérieur
                                {
                                    apport = apport + ((this.temperatureExt - piece.Temperature) * paroi.SurfaceReel) / paroi.CoefficientThermo;
                                }
                            }

                            if (piece.Radiateur) // On test radiateur
                            {
                                apport = apport + piece.CoefficientRadiateur;
                            }

                            int masseAir = piece.Surface * hauteur; // Calcule masseAir en fonction surface pièce
                            int intervalle;
                            ...
                            piece.Temperature = piece.Temperature + apportT; // On ajoute apport a la température
                        }
                ...
                if (ouvert != value)
                {
                    ...
                    if (StatusBDD == true)
                    {
                        try
                        {
                            ...
                            maRequete = "INSERT INTO domotique.dbo.LogPiece VALUES ('" + date + "', '" + etat + "', '" + id + "', '" + mode + "')";
    ...
    {
        public TPiece(string ANom, bool ARadiateur, double ATemperature, int ASurface, int ACoefficientRadiateur, ref TextBox ATextBoxT, ref TextBox ATextBoxC, ref Button AButtonChauffage, SqlConnection ACon, Label ALalbelHeure, Label ALabelDate, ref Boolean AStatusBDD, TMaison AMaison)
        {
            nom = ANom;
                ...
                if (temperature != value)
                {
                    ...
                    if (StatusBDD == true)
                    {
                        try
                       {
                            ...
                            maRequete = "Select temperature from domotique.dbo.LogT where date = (Select max(date) from domotique.dbo.LogT where FK_Piece = (Select PK_Piece from domotique.dbo.Piece where nom = '" + this.Nom + "' )  and FK_Mode = '"+mode+"' ) and FK_Piece = (Select PK_Piece from domotique.dbo.Piece where nom = '" + this.Nom + "')";
                            SqlCommand myCommand = new SqlCommand(maRequete, con);
                            double temperatureOld = Convert.ToDouble(myCommand.ExecuteScalar());
                            //Debug.Print(this.Nom + " : " +temperatureOld.ToString());
                            //Debug.Print(this.Nom + ": tempOld: " + temperatureOld + " - " + temperature + " = " + Math.Abs(temperatureOld - temperature));
                            if (Math.Abs(temperature - temperatureOld) > 1)
                            {
                                Debug.Print(this.Nom + ": tempOld: " + temperatureOld + " - " + temperature + " = " + Math.Abs(temperatureOld - temperature));
                            }
                            if (temperatureOld == 0 || Math.Abs(temperature - temperatureOld) > 1)
                            {
                                Debug.Print("passe bdd");
                                maRequete = "SELECT PK_Piece from domotique.dbo.Piece where nom = '" + this.Nom + "'";
                                SqlCommand myCommand2 = new SqlCommand(maRequete, con);
                                int id_piece = Convert.ToInt32(myCommand2.ExecuteScalar());
                                Debug.Print("piece: " + id_piece);
                                //Debug.Print("dans old");
                                maRequete = "INSERT INTO domotique.dbo.LogT VALUES ('" + date + "',  '"+ Convert.ToInt32(temperatureOld) + "' ,  '" + Convert.ToInt32(temperature) + "', '" + id_piece + "', '" + mode + "')";
                                myCommand = new SqlCommand(maRequete, con);
                                myCommand.ExecuteNonQuery();
